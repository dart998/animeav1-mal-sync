...
function resultTable(items,cacheMode=false){if(!items.length)return '<p>Sin elementos.</p>';const actionHead=cacheMode?'<th aria-label="Acciones"></th>':'';return '<div class="table-wrap"><table><thead><tr><th>AnimeAV1</th><th>MAL</th><th>Puntos</th><th>Episodios</th><th>Resultado</th><th>Detalle</th>'+actionHead+'</tr></thead><tbody>'+items.map(i=>{const action=cacheMode?'<td style="white-space:nowrap"><button type="button" class="secondary inspect-button" data-media-id="'+esc(i.media_id)+'" title="Ver candidatos">🔍</button> <button type="button" class="secondary recompute-button" data-media-id="'+esc(i.media_id)+'" title="Recalcular coincidencia">↻</button> <button type="button" class="danger trash-button" data-media-id="'+esc(i.media_id)+'" title="Eliminar esta coincidencia de la caché">🗑️</button></td>':'';const reverseUnmatched=i.result==='error'&&!i.media_id&&!!i.mal_id&&String(i.message||'').startsWith('sin coincidencia segura en AnimeAV1');const manual=i.result==='error'?(String(i.message||'').startsWith('Conflicto de episodios')?reverseConflictBox(i):(reverseUnmatched?reverseManualMatchBox(i):manualMatchBox(i))):'';return '<tr data-cache-row="'+esc(i.media_id)+'"><td>'+(i.media_id?(esc(i.source_title)+'<br>'+animeAV1IDLink(i.media_id)):'—<br><span class="muted">No identificado</span>')+'</td><td>'+esc(i.mal_title||'—')+(i.mal_id?' '+malIDLink(i.mal_id):'')+(i.mal_title_2?'<br>↳ '+esc(i.mal_title_2)+(i.mal_id_2?' '+malIDLink(i.mal_id_2):''):'')+'</td><td>'+esc(i.match_score||'—')+'</td><td>'+esc(i.from)+' → '+esc(i.to)+'</td><td>'+esc(i.result)+'</td><td>'+esc(i.message||'')+manual+'</td>'+action+'</tr>'}).join('')+'</tbody></table></div>'}
