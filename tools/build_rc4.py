...
replacement = r'''func animeAV1SlugFromRef(raw string) (string, error) {
	...
	if ref == "" { return "", errors.New("falta la URL o slug de AnimeAV1") }
	if strings.Contains(ref, "://") {
		u, err := url.Parse(ref)
		if err != nil { return "", errors.New("URL de AnimeAV1 no válida") }
		host := strings.ToLower(strings.TrimPrefix(u.Hostname(), "www."))
		if host != "animeav1.com" { return "", errors.New("la URL debe pertenecer a animeav1.com") }
		ref = u.Path
	} else {
	...
	if strings.HasPrefix(ref, "media/") { ref = strings.TrimPrefix(ref, "media/") }
	if i := strings.IndexByte(ref, '/'); i >= 0 { ref = ref[:i] }
	ref = strings.TrimSpace(ref)
	if ref == "" || strings.ContainsAny(ref, " ?#") { return "", errors.New("slug de AnimeAV1 no válido") }
	return ref, nil
}

func (a *App) reverseManualMatchAPI(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	if req.Method != http.MethodPost { http.Error(w, "POST", http.StatusMethodNotAllowed); return }
	if err := req.ParseForm(); err != nil { http.Error(w, err.Error(), http.StatusBadRequest); return }
	malID, _ := strconv.Atoi(strings.TrimSpace(req.FormValue("mal_id")))
	if malID <= 0 { http.Error(w, "mal_id es obligatorio", http.StatusBadRequest); return }
	...
	if err := a.malRequestContext(req.Context(), http.MethodGet, fmt.Sprintf("/anime/%d?fields=%s", malID, fields), nil, &anime); err != nil { http.Error(w, "ID de MAL no válido: "+err.Error(), http.StatusBadGateway); return }
	a.mu.Lock(); cookie := a.state.Settings.Cookie; a.mu.Unlock()
	queries := []string{strings.ReplaceAll(slug, "-", " "), anime.Title}
	seenQueries := map[string]bool{}
	var match animeAV1SearchItem
	for _, q := range queries {
		key := normalize(q); if key == "" || seenQueries[key] { continue }; seenQueries[key] = true
		items, searchErr := a.animeAV1Search(req.Context(), cookie, q); if searchErr != nil { continue }
		for _, candidate := range items { if strings.EqualFold(strings.Trim(candidate.Slug, "/"), slug) { match = candidate; break } }
		if match.ID != "" { break }
	}
	if match.ID == "" { http.Error(w, "no se encontró en AnimeAV1 una ficha con el slug "+slug, http.StatusNotFound); return }
	entry := CacheEntry{MediaID: match.ID, MALID: anime.ID, MALTitle: anime.Title, MatchType: "manual_reverse", MatchScore: 999, SourceTitle: normalize(match.Title), LastValidated: time.Now().Unix(), UpdatedAt: time.Now().Unix(), MatcherVersion: appVersion, SearchStrategy: "manual_animeav1_slug"}
